...
def get(panelist_id: int = None, question_id: str = None) -> Answer:
    with connect_session() as db:
        return (
            db.query(Answer)
            .filter(
                Answer.panelist_id == panelist_id, Answer.question_id == question_id
            )
            .first()
        )
...
def get_score(question_id: str = None, panelist_id: int = None) -> int:
    with connect_session() as db:
        return (
            ...
            .filter(
                and_(
                    Answer.question_id == question_id, Answer.panelist_id == panelist_id
                )
            )
            .scalar()
        )
...
def count_panelist_score(panelist_id: int = None) -> int:
    with connect_session() as db:
        return (
            ...
            .filter(Answer.panelist_id == panelist_id)
            .scalar()
        )
...
def get_team_answer_list(question_id: str = None) -> list[str, int]:
    with connect_session() as db:
        return (
            db.query(Panelist.team, func.avg(Answer.correct), func.avg(Answer.score))
...
def get_team_results() -> List[tuple[str, float]]:
    with connect_session() as db:
        team_counts = (
            db.query(Panelist.team, func.count(Panelist.id).label("team_count"))
            .group_by(Panelist.team)
            .subquery()
        )
